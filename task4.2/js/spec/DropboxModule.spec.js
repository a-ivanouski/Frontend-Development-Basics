describe('DropboxModule ', function() {
  ...
  beforeEach(inject(function(_dropboxService_ , _$httpBackend_){
    ...
    $httpBackend.whenPOST(urlForToken)
      .respond(function(method, url, data, headers){
        return [200 , responseAccessToken];
      });

    $httpBackend.whenGET(urlForProfileInfo)
      .respond(function(method, url, data, headers){
        expect(authorization).to.deep.equal(headers.Authorization);
        return [200 , responseProfileInfo];
      });

    $httpBackend.whenGET(uarForFolderInfo)
      .respond(function(method, url, data, headers){
        expect(authorization).to.deep.equal(headers.Authorization);
        return [200 , responseFolderInfo];
      });
